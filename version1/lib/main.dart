...
class _MyAppState extends State<MyApp> {
  ...
  void listenRecordingState() {
    _streamSubscription = _flutterBackgroundVideoRecorderPlugin.recorderState
        .listen((event) {
          switch (event) {
            case 1:
              _isRecording = true;
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(title: const Text('Background video recorder')),
        body: Center(
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                style: ElevatedButtonTheme.of(context).style?.copyWith(
                  backgroundColor: WidgetStateProperty.resolveWith(
                    (states) =>
                        (!_isRecording && !_recorderBusy) ? null : Colors.grey,
                  ),
                ),
                //------------------------------------------------------
                //changes the cameraFace and refreshes the widget tree
                onPressed: () {
              ...
              ElevatedButton(
                //----------------------------------------------------------------------------------
                //this function runs when the button "start recording" is clicked
                onPressed: () async {
                  if (!_isRecording && !_recorderBusy) {
                    try {
